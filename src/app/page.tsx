          ...
          <StatCard
            ...
            icon={
              <svg viewBox="0 0 20 20" fill="none" className="h-5 w-5">
                <path d="M10 10.5a3 3 0 100-6 3 3 0 000 6z" stroke="currentColor" strokeWidth="1.6" />
                <path d="M4 16.5c0-2.76 2.69-5 6-5s6 2.24 6 5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
              </svg>
            }
          />
          <StatCard
            ...
            icon={
              <svg viewBox="0 0 20 20" fill="none" className="h-5 w-5">
                ...
                <path d="M10 6.5V10.5L12.5 12" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            }
          />
          <StatCard
            ...
            icon={
              <svg viewBox="0 0 20 20" fill="none" className="h-5 w-5">
                <path d="M3.5 5.5A1.5 1.5 0 015 4h10a1.5 1.5 0 011.5 1.5v6A1.5 1.5 0 0115 13H8l-3.5 3V13H5a1.5 1.5 0 01-1.5-1.5v-6z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" />
              </svg>
            }
          />
          <StatCard
          ...
          <div className="grid gap-3 lg:grid-cols-3">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-300 py-2.5 pl-9 pr-3 text-sm outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/15"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/15"
              >
          ...
          {isLoading ? (
            <div className="space-y-3 p-6">
              {Array.from({ length: 5 }).map((_, index) => (
                <div key={index} className="animate-pulse rounded-xl bg-slate-100 px-4 py-4">
            ...
            <div className="p-10 text-center">
              <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-rose-50">
                <svg viewBox="0 0 20 20" fill="none" className="h-6 w-6 text-rose-500">
              ...
              <button
                ...
                className="mt-4 rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-500"
              >
                Retry
              </button>
            </div>
            ...
            <div className="p-10 text-center">
              <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-slate-100">
                <svg viewBox="0 0 20 20" fill="none" className="h-6 w-6 text-slate-400">
                  <circle cx="9" cy="9" r="6" stroke="currentColor" strokeWidth="1.6" />
                  <path d="M13.5 13.5L17 17" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
                </svg>
              </div>
              ...
              <div className="hidden overflow-x-auto md:block">
                <table className="min-w-full divide-y divide-slate-100 text-left text-sm">
                  <thead className="bg-slate-50/80">
                    <tr>
                      {["Lead", "Phone", "School/Org", "City", "Source", "Enquiry", "Status", "Date", "Actions"].map((h) => (
                      ...
                      return (
                        <tr key={lead.id} className="align-top transition hover:bg-slate-50/60">
                          <td className="px-4 py-4">
                            <div className="flex items-center gap-3">
                              <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-bold ${avatarColor(lead.fullName)}`}>
                          ...
                          <td className="px-4 py-4">
                            <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${sourceStyles[lead.source] || "bg-slate-100 text-slate-700"}`}>
                              {lead.source || "—"}
                            </span>
                          </td>
                          ...
                          <td className="px-4 py-4">
                            <div className="flex flex-col gap-2">
                              <select
                                ...
                                className="rounded-lg border border-slate-300 px-2 py-2 text-xs outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/15"
                              >
                                <option value="new">New</option>
                                <option value="contacted">Contacted</option>
                                <option value="converted">Converted</option>
                              ...
                              {lead.message ? (
                                <button
                                  type="button"
                                  onClick={() =>
                                    setExpandedMessages((current) => ({ ...current, [lead.id]: !current[lead.id] }))
                                  }
                              ...
                              {lead.message && expandedMessages[lead.id] ? (
                                <div className="max-w-xs rounded-xl bg-slate-50 p-3 text-xs leading-relaxed text-slate-700">
                                  {lead.message}
                                </div>
                              ) : null}
                            </div>
                  ...
                  return (
                    <article key={lead.id} className="rounded-2xl border border-slate-200 p-4 shadow-sm">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3">
                          <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-sm font-bold ${avatarColor(lead.fullName)}`}>
                            {initials(lead.fullName)}
                          </div>
                          <div>
                      ...
                      <div className="mt-3 grid gap-1.5 text-sm text-slate-600">
                        ...
                        <p>
                          ...
                          <span className={`ml-1 inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${sourceStyles[lead.source] || "bg-slate-100 text-slate-700"}`}>
                            {lead.source || "—"}
                          </span>
                        </p>
                      ...
                      <div className="mt-4 flex flex-col gap-2">
                        <select
                          ...
                          onChange={(event) => handleStatusChange(lead.id, event.target.value)}
                          disabled={pendingStatusId === lead.id}
                          className="rounded-lg border border-slate-300 px-2 py-2 text-sm outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/15"
                        >
                          <option value="new">New</option>
                          <option value="contacted">Contacted</option>
                          <option value="converted">Converted</option>
                        ...
                        {lead.message && expandedMessages[lead.id] ? (
                          <div className="rounded-xl bg-slate-50 p-3 text-sm leading-relaxed text-slate-700">
                            {lead.message}
                          </div>
                        ) : null}
                      </div>
